...
def build_mosaic_exact_size(
    ...
    for gy in range(grid_h):
        ...
        for gx in range(grid_w):
            ...
            best_i = find_best_tile(rgb, tile_avgs, index)
        ...
        if gy % 1 == 0:
            _set("jobs", job_id, progress=int(done / total_cells * 100))

    out.save(out_path, quality=92)
    _set("jobs", job_id, status="done", progress=100, message="Done!", result_path=str(out_path))


def run_job(job_id: str, target_path: Path, material_id: str, tile_size: int):
    try:
...
async def create_job(
    ...
    th = threading.Thread(target=run_job, args=(job_id, target_path, material_id, tile_size), daemon=True)
    th.start()
